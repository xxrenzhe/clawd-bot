...
function buildOfflineBody(
  ...
  lines.push('');
  lines.push('## Free Installation Service');
  lines.push('We offer a free Clawdbot installation service that includes environment checks, baseline configuration, and first-run guidance. Schedule it via the [Contact page](/contact).');
  lines.push('');
...
8. **SERVICE CTA**: Include a section that mentions "free Clawdbot installation service" with a link to /contact
...
function validateGeneratedContent(content: string, topic: ArticleTopic): { valid: boolean; issues: string[] } {
  ...
  if (!content.includes('free Clawdbot installation service')) {
