...
[Embed Query] → Vector representation
    ↓
[Search Vector Store] → Top-k similar documents
    ↓
[Rerank Results] → Sort by relevance
    ↓
[Build Context] → Combine with system prompt
    ↓
[Generate Response] → LLM creates answer
    ↓
[Return with Sources] → Include citations
```
...
Collections: 3
├─ product-docs: 1,234 chunks (45 documents)
├─ support-kb: 567 chunks (23 documents)
└─ internal: 890 chunks (12 documents)
